...
class BeautyContestUI(tk.Tk):
    ...
    def create_buttons_compact(self):
        btn_0 = tk.Button(self.frame_grid, text="0", font=config.FONT_NORMAL, width=4, height=1,
        ...
        for i in range(10):
            for j in range(10):
                val = (i * 10) + (j + 1)
                btn = tk.Button(self.frame_grid, text=str(val), font=config.FONT_NORMAL, width=3, height=1,
    ...
    def animate_step(self, step):
        ...
        if step == 0:
            ...
            self.log(f"Target (x0.8): {res['target']}", bold=True) 
            if res['logs']:
                for l in res['logs']:
